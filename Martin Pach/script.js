$(document).ready(function() {
    ...
    function getSelectedMonth(date) {
        if (date.charAt(5) == '1' && date.charAt(6) == '0') {
            return '9';
        } else if (date.charAt(5) == '0') {
            return date.charAt(6) - 1;
        } else {
            return date.charAt(5) + (date.charAt(6) - 1);
        }
    };

    function getSelectedDay(date) {
        var selectedDay = (date.charAt(8) == '0') ? date.charAt(9) : date.charAt(8) + date.charAt(9);
        return selectedDay;
    };

    $('#date').on('change', function() {
        var selectedDate = $('#date').val();
        $('#nameday_date').text(selectedDate);
        $('#nameday_name').text(jsData[getSelectedMonth(selectedDate)][getSelectedDay(selectedDate)]);
    });
});
